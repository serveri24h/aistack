import torch
from diffusers import StableDiffusion3Pipeline, StableDiffusionPipeline
import os
import PIL
import datetime

IMAGEPATH = '/app/generated'

def run_stable_diffusion3(prompt:str):
    os.environ['PYTORCH_CUDA_ALLOC_CONF'] = 'max_split_size_mb:128'

    pipe = StableDiffusion3Pipeline.from_pretrained("stabilityai/stable-diffusion-3-medium-diffusers", torch_dtype=torch.float16, safety_checker = None)
    pipe = pipe.to("cuda")
    print("PIPE IS DONE")
    image = pipe(
        prompt,
        negative_prompt="",
        num_inference_steps=28,
        guidance_scale=7.0,
    ).images[0]
    print("IMAGE IS DONE", type(image))
    image.save(f"{IMAGEPATH}/testin.png")



def run_stable_diffusion(prompt:str):
    print("HW1")
    pipe = StableDiffusionPipeline.from_pretrained("runwayml/stable-diffusion-v1-5", torch_dtype=torch.float16, safety_checker = None)
    print("HW2")
    pipe = pipe.to("cuda")
    print("HW3")
    image:PIL.Image.Image = pipe(prompt).images[0]
    print("HW4")
    image.save(f"{IMAGEPATH}/generated_image_{datetime.datetime.now()}.png")
    print("HW5")